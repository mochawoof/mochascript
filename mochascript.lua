...
function teleportToBlip(id, name, onlyMine, offset)
    local success = false
    local blip = 0
    if (onlyMine) then
        blip = getMyBlip(id)
    ...
    if (blip != 0) then
        ...
        players.teleport_3d(players.user(), pos.x + offset.x, pos.y + offset.y, pos.z + offset.z + 50)
        util.yield(800)
...
local factEveryMins = factEvery / (1000 * 60)
local lastFact = 0
function chatRandom(prefix, src)
    local time = util.current_time_millis()
    if (time - lastFact > factEvery) then
        chat.send_message(prefix .. src[math.random(1, #src)], false, true, true)
        lastFact = time
    end
end

local myself = menu.my_root():list_action("Self", {}, "", {
    {1, "Copy my position"},
}, function(value, menu_name, click_type)
    switch value do
        case 1:
            local pos = players.get_position(players.user())
            local formatted = math.floor(pos.x) .. ", " .. math.floor(pos.y) .. ", " .. math.floor(pos.z)
            util.copy_to_clipboard(formatted)
            util.toast("Copied " .. util.get_clipboard_text())
            break
    end
end)
...
online:list_action("Teleport", {}, "", {
    ...
    {4, "Hangar"},
    {5, "Business battle"},
    {6, "Beast", {}, "Only works when Beast's blip is visible"}
}, function(value, menu_name, click_type)
    ...
    switch value do
        case 1:
            ...
            break
        case 2:
            teleportToBlip(614, menu_name, true, kindaUp)
            break
        case 3:
            teleportToBlip(475, menu_name, true, kindaUp)
            break
        case 4:
...
menu.my_root():hyperlink("v1.0", "https://github.com/mochawoof/mochascript")